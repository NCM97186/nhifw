...
<li class="current"><a href="<?php echo $HomeURL;?>/hi" title="मुख्य पृष्ठ">मुख्य पृष्ठ</a></li>
	<?php 
		if($mydb->checkTable_threeRow("menu_publish","m_flag_id",0,"menu_positions",1,"approve_status",3)>0){
			...
			$whereClause="m_flag_id='0' && menu_positions='1' && approve_status='3' && language_id='2' order by page_postion asc" ;
...
?>
	<?php foreach($leftrows as $key=>$value){
			 ...
			 $whereClause="m_flag_id='".$value['m_publish_id']."' && menu_positions='1' && approve_status='3' && language_id='2' order by page_postion asc" ;
			...
			$row2 = mysqli_num_rows($sql1);
				 if($row2 > 0){ ?>
			<li class="<?php if($num1 >0) {echo "has-sub";} echo $class;?>"><a href="#" title="<?php echo $value['m_name'];?>"><?php echo $value['m_name'];?></a> 
