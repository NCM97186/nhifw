...
?>
	<?php foreach($leftrows as $key=>$value){
			...
			$sql1 = mysqli_query($conn,"select * from student_publish where m_flag_id='".$value['m_publish_id']."' and language_id='2' and approve_status='3' ORDER BY page_postion ASC");
			//echo $value['m_name']."".
			...
			<ul  class="ddsubmenustyle blackwhite">
			  ...
			  $sql2 = mysqli_query($conn,"select * from student_publish where m_flag_id='".$value1['m_publish_id']."' and language_id='2' and approve_status='3' ORDER BY page_postion ASC");
			...
			if($value3['doc_uplode']=='') {?>
			  <li    class="collapsed expandable"><a href="<?php echo $HomeURL;?>/hi/student/<?php echo $value3['m_url']; ?>" title="<?php echo $value3['m_name'];?>"><?php echo $value3['m_name'];?></a></li>
			<?php } else { ?>
			...
			<?php }
			 ...
			 <?php } ?>
				...
				elseif($row3 == 0 ){
		?>
		<li><a href="<?php echo $HomeURL;?>/hi/student/<?php echo $value1['m_url']; ?>" title="<?php echo $value1['m_name'];?>"><?php echo $value1['m_name'];?></a></li>
		<?php	 } 
				}  ?>
			</ul>
	...
	elseif($row2 == 0 ){ ?>
		...
		<li><a href="<?php echo $HomeURL;?>/hi/student/<?php echo $value['m_url']; ?>" title="<?php echo $value['m_name'];?>"><?php echo $value['m_name'];?></a></li>
<?php	}  }?>
			</ul>
