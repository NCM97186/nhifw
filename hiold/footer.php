...
<?php foreach($leftrows as $key=>$value){
			...
			$whereClause="m_flag_id='".$value['m_publish_id']."' && menu_positions='3' && approve_status='3' && language_id='2' order by menu_positions asc limit 0,5" ;
		...
		if($mydb->checkTable_threeRow("menu_publish","m_flag_id",0,"menu_positions",1,"approve_status",3)){
		
			$whereClause="m_flag_id='0' && menu_positions='1' && approve_status='3' && language_id='1' and m_publish_id='228'order by page_postion asc limit 0,4";
			$leftrows=$mydb->gettable_Rows_whereCluse("menu_publish",$whereClause); 
			//$leftrows=$mydb->gettable_RowsthreeColumn_where("menu_publish","m_flag_id",0,"menu_positions",1,"approve_status",3);
			//$num=$mydb->countTableRow("menu_publish","m_flag_id",$value['m_publish_id'],"menu_positions",1,"approve_status",3);
		 }
