...
require_once "../../includes/functions-data.php";
 $page12=base64_decode($_GET['page']);
...
if($mydb->checkTable_threeRow("project_publish","m_url",$url,"approve_status",3,"language_id",2)>0){
			$contentrows=$mydb->gettable_RowsthreeColumn_where("project_publish","m_url",$url,"approve_status",3,"language_id",2);  
...
foreach($contentrows as $key=>$value){ 
			...
			$position=$value['menu_positions'];
			 $rootid1=get_project_parent($page_id);
			 $m_name=get_project_main($page_id);
		$m_url=$value['m_url'];
...
<div class="left-menu">
<div class="left">
<?php include("../left_inner_project_menu.php");?>
...
<div class="col-md-9 content-area">
			...
			$whereClause="m_flag_id='8' && approve_status='3' && language_id='2' order by page_postion asc" ;
			$leftrows11=$mydb->gettable_Rows_whereCluse("project_publish",$whereClause); 
?>
              <ul>
