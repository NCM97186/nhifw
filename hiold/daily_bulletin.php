...
<div class="row">
   ...
   <li class = "active"><a href="<?php echo $HomeURL;?>/hi/daily_bulletin.php" title="Daily Health News Bulletin">Daily Health News Bulletin</a></li>
    <?php if($_GET['page']!='') { ?> 
  ...
  <table  class="table table-bordered">
			   ...
			   <th>Last Date</th>
			    ...
			    <?php if($mydb->checkTableRow("importent_information_publish")>0){
					$i=1;
					$date=date('Y-m-d');
				$whereClause="approve_status='3' && language_id='2' && cat_id='2' && date(end_date ) <= '$date'  order by start_date desc" ;
			   ...
			   <?php } else { ?>
			   <a href="<?php echo $HomeURL;?>/hi/important_circulars.php?page=<?php echo $value['page_url']; ?>" title="<?php echo $value['m_name'];?>"><?php echo $value['m_name'];?></a>
			   <?php } ?>
			   </td>
