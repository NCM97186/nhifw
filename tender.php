...
<div class="col-md-9 content-area">
  ...
  <table  class="table table-bordered">
			   ...
			   <th>Last Date</th>
			    ...
			    <?php if($mydb->checkTableRow("tender_publish")>0){
					$i=1;
				//$newsrows=$mydb->gettable_RowsTwoColumn_where("tender_publish","approve_status",3,"language_id",1); 
				$date=date('Y-m-d');
			$whereClause="approve_status='3' && language_id='1'  && date(end_date ) >= '$date'  order by start_date desc" ;
			$tenderrows=$mydb->gettable_Rows_whereCluse("tender_publish",$whereClause);
			if(is_array($tenderrows)){
					  $no_of_rows= count($tenderrows);
					 }else{
					  $no_of_rows= $tenderrows;
					}
				}
				?>
				 <?php
					 if($no_of_rows > 0){
					foreach($tenderrows as $key=>$value){ 
					 $docspath = $HomeURL.'/upload/tender/'.$value['docs_file'];
					 $file='../upload/tender/'.$value['docs_file'];
				?>
			   <tr>
			   <td><?php echo $i;?></td>
			   <td>
			   ...
			   <?php } else { ?>
			   <a href="<?php echo $HomeURL;?>/tender.php?page=<?php echo $value['page_url']; ?>" title="<?php echo $value['m_name'];?>"><?php echo $value['m_name'];?></a>
			   <?php } ?>
			   </td>
