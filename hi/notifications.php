...
if($mydb->checkTable_threeRow("importent_information_publish","page_url",$url,"approve_status",3,"language_id",2)>0){
			$contentrows=$mydb->gettable_RowsthreeColumn_where("importent_information_publish","page_url",$url,"approve_status",3,"language_id",2);  
...
$api_url = 'http://125.20.102.83/nihfw_recruitment/api/notifications/';
...
<div class="col-md-9 content-area">
                <h2 class="heading"> <?php if($_GET['page']!='') { ?> 
					<?php echo $page_name;?>
				 <?php }  else { ?>
				*** All Notifications/Advertisements Details ***
				<?php } ?></h2>
          <?php if($_GET['page']=='') { ?>           
  ...
  <table  class="table table-bordered">
			   ...
			   <th>Apply	</th>
				...
				foreach($response_data as $value){
					...
					//echo $value->link_to_pdf;

					 $docspath = 'http://125.20.102.83/nihfw_recruitment/uploads/link_to_pdf/'. $value->link_to_pdf;
					 $file='upload/importent/'.$value->link_to_pdf;
					 $size = formatFilebytes($file,'MB');

				?>
			   <tr>
			   <td><?php echo $i;?></td>
			   <td><?php echo $value->post_name ;?></td>
			   <td><?php echo $value->adver_no;?></td>
			   <td><?php echo date("d-m-Y", strtotime($value->start_date));?></td>
			   <td><?php echo date("d-m-Y", strtotime($value->last_date));?></td>
			   <td>
			   <?php if($value->link_to_pdf!='') { ?>
			   <a href="<?php echo $docspath; ?>" target="_blank"> View Advertisement &nbsp;&nbsp;<img src="<?php echo $HomeURL; ?>/images/pdf_icon.png" height="16" alt="PDF file" data-name="" /> </a>
			   <?php }
				 ?>
		     	</td>
			<td><a class="btn btn-success" href="http://125.20.102.83/nihfw_recruitment/user/login"><i class="fa fa-check-circle-o" aria-hidden="true"></i> Apply</a></td>
			   </tr>
			  <?php $i++; } } else { ?>
