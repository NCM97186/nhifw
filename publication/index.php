...
if($_SERVER['REQUEST_URI'])
		...
		$url1=$val['2'];
		$open=$val['1'];
		 if($mydb->checkTableRow("publication_category")>0){
		 $i=1;
		$date=date('Y-m-d');
			 $whereClause="status='1' && language_id='1' && page_url='".$url1."'" ;
					
					$newsrows=$mydb->gettable_Rows_whereCluse("publication_category",$whereClause);
...
<div class="left-menu">
<div class="left">
<?php include("../publication_menu.php");?>
...
<div class="col-md-9 content-area">
			   ...
			   <th>Last Date</th>
			    ...
			    <?php if($mydb->checkTableRow("publication_publish")>0){
					$i=1;
					$date=date('Y-m-d');
			...
			$whereClause="approve_status='3' && language_id='1' && category='".$cat."'    order by start_date desc" ;
					...
					$newsrows=$mydb->gettable_Rows_whereCluse("publication_publish",$whereClause);
