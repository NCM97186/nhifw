...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
 			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM tbl_training WHERE m_id=$cid";
					$sql="Delete From tbl_training where m_id='$deleteid'";
					$res=mysqli_query($conn,$sql);
					$page_id=mysqli_insert_id($conn);
					$sql1="Delete From tbl_training_publish where m_publish_id='$deleteid'";
...
// 					if($res)
// 					{	
					header("location:delete.php?status=latest_deleteid");
					// }
...
if(($inactiveid !=''))
{
   ...
   $sql="Update tbl_training set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update tbl_training_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Important Information : <?php echo $sitename;?></title>
...
<?php 
	if($btnsubmit=="Search")
	{
				...
				$texcat1 =content_desc(check_input($_POST['texcat']));
	...
	if($btneng!='')
	  ...
	  {
	 $date=date('Y-m-d');
  $query ="select * from tbl_training where 1  $querywhere    ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
$wherecluse=" and language_id=$language and approve_status='$txtstatus1'";
}
$date=date('Y-m-d');
  $query ="select * from tbl_training where 1 $wherecluse   ORDER BY page_postion ASC ";
}
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Training</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="add_training.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
<?php if($role_map['medit']=='ED' || $user_id=='101'){?><li id="editer" class="edit-icon"><a href="javascript:void(0);" onclick="alert('Please select atleast one record')" title="Edit">Edit</a><?php }?></li>
             <?php if($role_map['mdelete']=='DE' || $user_id=='101'){?> 
			 <?php if ($txtstatus!='3'){?>
               <li id="delete" class="delete-icon"><a href="javascript:void(0);" onclick="alert('Please select atleast one record')" title="Delete">Delete</a></li> <?php } } ?>   
            </ul>
...
<option value="1" <?php if ($texcat=='1') echo 'selected="selected"';?>   >Institutional Training</option>
<option value="2"  <?php if ($texcat=='2') echo 'selected="selected"';?>  >Central Training Plan</option>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_training.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('trainingview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=tbl_training'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="add_training.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_training.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
