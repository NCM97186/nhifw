...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql1="Delete From student_publish where m_publish_id='$deleteid'";
...
if(($inactiveid !=''))
{
 ...
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
	header("location:delete.php?status=student_inactiveid");
	}
}
...
<div class="breadcrum">
	<span class="submenuclass"><a href="welcome.php">Dashboard </a> </span>
			...
			<span class="submenuclass">Manage Student Corner</span>
	...
	if($btnsubmit=="Search" || $_GET['page_query'] =="Search")
	{
	  ...
	  if($btneng!='')
	  {
		...
		$_SESSION['lname']=$btneng;
			...
			$querywhere .=" and student.language_id=$language";
	  }
	  if($errmsg=='')	
	  {
	     if($txtstatus=='1' || $txtstatus=='2'){
		  $query ="select m_id,m_name,m_title,approve_status from student  where 1 $querywhere ORDER BY page_postion ASC";
		 }else { $query ="select m_id,m_name,m_title,approve_status from student  where m_flag_id=0 $querywhere ORDER BY page_postion ASC"; }
	  }
...
//$query ="select m_id,m_name,m_title,language_id,approve_status from student $wherecluse  ORDER BY page_postion  ASC";

$query ="select m_id,m_name,m_title,language_id,approve_status from student ORDER BY page_postion  ASC";
...
<a href="add_student.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<input id="filter_search" type="text" title="Search title or student Name." value="<?php echo content_desc($_POST['filter_search']);?>" name="filter_search">
...
<div id="response"> </div>
							...
							<span class="space-pagetitle"><?php if(rootstudent($m_id) >0) {?><a href="manage_substudent.php?sub=<?php echo $m_id; ?>&&txtstatus=<?php echo $txtstatus;?>" title="Sub project View"><?php status($approve_status);?></a><br /><span class="clicksubmenu">(Click to View Subproject)</span><?php } else { status($approve_status); }?></span>
							
							<span class="space-lang"><?php language($language_id); ?></span>

							<span class="space-option">
                            <?php  if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_student.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('studentview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=700,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php  }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="edit_student.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
