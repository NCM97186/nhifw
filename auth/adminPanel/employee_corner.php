...
$cat_id='1';
$tableName_send="employee_corner";
...
if($txtstatus=='3')
{
$tableName_send="employee_corner_publish";
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
		...
		if ($_FILES["txtuplodepdf"]["name"]!="")
			{
			$sql = "select docs_file FROM employee_corner WHERE id=$cid";
			$rs = mysqli_query($conn,$sql);
			$row=mysqli_fetch_array($rs);
			$image_path = "../../upload/employee/".$row['docs_file'];
			unlink($image_path);
...
if($txtstatus=='3'){
	...
	$sql=mysqli_query($conn,"Select * from employee_corner_publish $whereclause");
...
$value=$useAVclass->insertQuery($tableName,$tableFieldsName_send,$tableFieldsValues_send);
	...
	$page_id=$cid;
	$tableName_send="employee_corner_publish";
	$tableFieldsName_send=array("language_id","publish_id","category","m_description","m_title","m_content","page_url","ext_url","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date");
	$tableFieldsValues_send=array("$txtlanguage","$page_id","$category","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from employee_corner where id='".$_GET['editid']."'");
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_employee_corner.php" title="Manage What's New">Manage Employee Corner</a></span>
			 <span class="submenuclass">>> </span> 
...
<div class="frm_row"> <span class="label1">
     ...
     <div class="frm_row"> <span class="label1">
				...
				<?php 
				$sqlquery=mysqli_query($conn,"select * from emp_category where status='1'");
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_employee_corner.php';" />
         </span>
              <div class="clear"></div>
            </div>
