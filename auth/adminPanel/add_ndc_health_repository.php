...
$cat_id='1';
$tableName_send="ndc_health_repository";
...
if($txtstatus=='3')
{
$tableName_send="ndc_health_repository_publish";
...
header("location:manage_ndc_health_repository.php");	
exit;	
}
}
if(isset($_POST['cmdsubmit']) && $_GET['editid']!='')
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
		{
			$sql = "select docs_file FROM ndc_health_repository WHERE id=$cid";
...
$sql=mysqli_query($conn,"Select * from ndc_health_repository_publish $whereclause");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from ndc_health_repository where id='".$_GET['editid']."'");
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_ndc_health_repository.php" title="Manage What's New">Manage NDC  </a></span>
			 <span class="submenuclass">>> </span> 
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_ndc_health_repository.php';" />
         </span>
              <div class="clear"></div>
            </div>
