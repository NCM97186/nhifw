...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql="Delete From employee_corner where id='$deleteid'";
...
if(($inactiveid !=''))
{
   ...
   $sql="Update employee_corner set approve_status='1' where id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update employee_corner_publish set approve_status='1' where publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Employee Corner : <?php echo $sitename;?></title>
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Employee Corner</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="employee_corner.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<span class="space-pagetitle" style="width: 15% !important;"><?php status($approve_status); ?></span>
							<span class="space-option" style="width: 12% !important;" ><?php if ($category == '2') {
								echo 'Circulars and Notifications';
							}elseif ($category == '3') {
								echo 'Accounts';
							}elseif ($category == '4') {
								echo 'Admin';
							...
							elseif ($category == '9') {
								echo 'Press and Rep  Unit';
							}
							elseif ($category == '10') {
								echo 'Workshop and Maintenance Section';
							}
							elseif ($category == '11') {
								echo 'Other';
							}
							...
							<span class="space-option" style="width: 12% !important;" ><?php language($language_id); ?></span>

							<span class="space-option">
                            <?php  if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_employee_corner.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('employeeview.php?page_id=<?php echo base64_encode($id);?>','window','width=1000,height=600,scrollbars=ye	s')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="employee_corner.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_employee_corner.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
