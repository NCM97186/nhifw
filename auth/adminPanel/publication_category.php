...
if(isset($_POST['Submit_g']) && $_GET['id']=='')
	 ...
	 if($_SESSION['logtoken']!=$_POST['random'])
			...
			$tableName_send="publication_category";
			...
			$tableFieldsValues_send=array("$txtename1","$a_status1","$txtlanguage","$r_url");
					...
					$msg=CONTENTADD;
					$_SESSION['SESS_MSG']=$msg;
					header("location:publication_category.php");
					exit;	
			}
 ...
 if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($did))))
	{
		...
		setcookie("Temp",$_SESSION['saltCookie']);
		$_SESSION['logtoken'] =md5(uniqid(mt_rand(), true));
		mysqli_query($conn,"delete from publication_category where id='$did'");
...
<title>Manage Publication Category: NIHFW</title>
...
<a href="institution_trade.php" title="New"><span class="icon-28-new"></span>New</a></li>
	...
	if($_GET['id']!='')
	{
		$rq = mysqli_query($conn,"select * from publication_category where id='".base64_decode($_GET['id'])."'");
...
<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2" style="border:1px solid #cccccc;padding: 10px;">
	...
	<?php 
$columns = "select * ";
$sql = "from publication_category where 1 ";
...
// $rows = mysqli_query($conn,$sql);
	...
	if($rows==0) { ?>
    <tr><td style="color:#F00;" height="30" align="center" colspan="7"><b>Sorry.. No records available.</b></td></tr>
<?php	}else	{	?>
...
?>
  <tr valign="top" onMouseMove="javascript: this.style.background='#ECF1F2'" onMouseOut="javascript: this.style.background='#FFFFFF'">
    ...
    <td width="47" align="center" class="left-tdtext"><a href="publication_category.php?id=<?php echo base64_encode($data['id']);?>" class="bluelink"><input type="image" border="0" alt="Edit" src="images/edit.png"  title="Edit" /></a></td>
    <td width="63" align="center" class="left-tdtext"><a href="publication_category.php?did=<?php echo $data['id'];?>&random=<?php echo $_SESSION['logtoken'];?>" class="bluelink" onClick="return confirm('Are you sure you want to delete record')"><input type="image" border="0" alt="Delete" src="images/deletes-icon.png"  title="Delete" /></a></td>
  </tr>
<?php	}?>
	<tr>
