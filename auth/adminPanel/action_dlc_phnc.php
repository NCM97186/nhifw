...
if($txtstatus=='3')
{
$tableName_send="dlc_phnc_publish";
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
		...
		if ($_FILES["txtuplodepdf"]["name"]!="")
			{
			$sql = "select docs_file FROM dlc_phnc WHERE id=$cid";
...
$sql=mysqli_query($conn,"Select * from dlc_phnc_publish $whereclause");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from dlc_phnc where id='".$_GET['editid']."'");
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_dlc_phnc.php" title="Manage What's New">Manage Employee Corner</a></span>
			 <span class="submenuclass">>> </span> 
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_dlc_phnc.php';" />
         </span>
              <div class="clear"></div>
            </div>
