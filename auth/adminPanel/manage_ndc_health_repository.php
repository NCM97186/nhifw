...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql="Delete From ndc_health_repository where id='$deleteid'";
					$res=mysqli_query($conn,$sql);
					$page_id=mysqli_insert_id($conn);
					$sql1="Delete From ndc_health_repository_publish where publish_id='$deleteid'";
					...
					// if($res)
					// {	
					header("location:manage_ndc_health_repository.php?status=employee_deleteid");
...
if(($inactiveid !=''))
{
   ...
   $sql="Update ndc_health_repository set approve_status='1' where id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update ndc_health_repository_publish set approve_status='1' where publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
		
	header("location:manage_ndc_health_repository.php?status=employee_inactiveid");
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Health Repositroy : <?php echo $sitename;?></title>
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
  $query ="select * from ndc_health_repository where 1 $querywhere ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
$wherecluse=" language_id=$language and approve_status='$txtstatus'";
  ...
  $query ="select * from ndc_health_repository where $wherecluse ORDER BY page_postion ASC ";
...
<h3 class="manageuser">Manage Health Repository </h3>
<div class="right-section">
<ul>
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="add_ndc_health_repository.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_ndc_health_repository.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="javascript:void(0);" class="cat_link" title="View" onclick="MM_openBrWindow('ndc_repository_view.php?page_id=<?php echo base64_encode($id);?>','window','width=1000,height=600,scrollbars=yes')"> View </a>
							</span>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="add_ndc_health_repository.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_ndc_health_repository.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
