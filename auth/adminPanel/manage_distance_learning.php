...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql1="Delete From distance_learning_publish where m_publish_id='$deleteid'";
					...
					header("Location:delete.php?status=distance_deleteid");
			}
			else
			{
			/*session_unset($admin_auto_id_sess);
...
if(($inactiveid !=''))
{
 ...
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
	header("location:delete.php?status=distance_inactiveid");
	}
}
...
<div class="breadcrum">
	<span class="submenuclass"><a href="welcome.php">Dashboard </a> </span>
			...
			<span class="submenuclass">Manage Distance Learning</span>
...
<?php 
	if($btnsubmit=="Search" || $_GET['page_query'] =="Search")
	{
		...
		if($filter_search!='')
		{
				if(preg_match("/^[aA-zZ][a-zA-Z -]{1,20}+$/", trim($filter_search)) == 0)
				{
	  ...
	  if($btneng!='')
	  {
		...
		$_SESSION['lname']=$btneng;
			...
			$querywhere .=" and distance_learning.language_id=$language";
	  }
	  if($errmsg=='')	
	  {
	     if($txtstatus=='1' || $txtstatus=='2'){
		  $query ="select m_id,m_name,m_title,approve_status from distance_learning  where 1 $querywhere ORDER BY page_postion ASC";
		 }else {
		 
		   $query ="select m_id,m_name,m_title,approve_status from distance_learning  where m_flag_id=0 $querywhere ORDER BY page_postion ASC"; }
	  }
...
//$query ="select m_id,m_name,m_title,language_id,approve_status from distance_learning $wherecluse  ORDER BY page_postion  ASC";
$query ="select m_id,m_name,m_title,language_id,approve_status from distance_learning ORDER BY page_postion  ASC";
...
<input id="filter_search" type="text" title="Search title or distance_learning Name." value="<?php echo content_desc($_POST['filter_search']);?>" name="filter_search">
...
<div id="response"> </div>
							...
							<?php	
							$pager = new PS_Pagination($conn, $query,"txtstatus=$txtstatus&page_query=Search");
							$rs = $pager->paginate($conn, $query,"txtstatus=$txtstatus&page_query=Search");
							...
							<span class="space-pagetitle"><?php if(rootdistance($m_id) >0) {?><a href="manage_subdistance_learning.php?sub=<?php echo $m_id; ?>&&txtstatus=<?php echo $txtstatus;?>" title="Sub project View"><?php status($approve_status);?></a><br /><span class="clicksubproject">(Click to View Subproject)</span><?php } else { status($approve_status); }?></span>
							
							<span class="space-lang"><?php language($language_id); ?></span>

							<span class="space-option">
                            <?php  if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_distance_learning.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('distance_learningview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=700,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php  }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="edit_distance_learning.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
