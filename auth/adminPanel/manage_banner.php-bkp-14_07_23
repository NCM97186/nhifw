...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql1="Delete From banner_publish where publish_id='$deleteid'";
...
if(($inactiveid !=''))
{
 ...
 $res=mysqli_query($conn,$sql);
$sql="Update banner_publish set  approve_status='1' where publish_id ='$inactiveid'"; 
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
...
<?php if($btnsubmit=="Search")
{
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
		 $query ="select b_id as id ,b_name as name,b_title as title, b_language as language_id,approve_status from banner  where 1 $querywhere  ORDER BY page_postion ASC";
	  }
	 else
	{
...
$wherecluse="where b_language=$language and approve_status='$txtstatus'";
 ...
 $query ="select b_id as id ,b_name as name,b_title as title, b_language as language_id,approve_status from banner where 1  ORDER BY page_postion ASC";
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="homebanner.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
