...
$tableName_send="importent_information";
$tableFieldsName_old=array("language_id","cat_id","m_name","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","c_new_status","create_date","docs_file","ext_url","content_type","m_keyword","m_description");
$tableFieldsValues_send=array("$txtlanguage","$cat_id","$txtename","$sortcontentdesc","$m_title","$txtcontentdesc","$url","$startdate","$expairydate","$txtstatus","$user_id","$newicons","$createdate","$txtuplodepdf","$txtweblink","$texttype","$txtekeyword","$txtmeta_description");
...
if($txtstatus=='3')
{
	$sql="INSERT INTO importent_information_publish (m_publish_id,language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description)
	 SELECT m_id, language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description
   FROM importent_information WHERE m_id=$page_id";
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
			...
			$sql = "select docs_file FROM importent_information WHERE m_id=$cid";
	...
	$whereclause="m_id=$cid";
$tableName_send="importent_information";
$old=array("language_id","m_name","cat_id","m_short","m_title","m_content","page_url","start_date","end_date","approve_status","admin_id","c_new_status","create_date","ext_url","content_type","m_keyword","m_description","update_date");
$new=array("$txtlanguage","$txtename","$cat_id","$sortcontentdesc","$m_title1","$txtcontentdesc","$url","$startdate","$expairydate","$txtstatus","$user_id","$newicons","$create_date","$txtweblink","$texttype","$txtekeyword","$txtmeta_description","$update_date");
	$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
...
$sql=mysqli_query($conn,"Select * from importent_information_publish $whereclause");
$row=mysqli_num_rows($sql); 

	if($create_version==1 and $row >0){ 
	$sql="INSERT INTO importent_information_publish_versions (m_publish_id,language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,image_file,docs_file,ext_url,content_type,more_photo,m_keyword,m_description,create_versions_date,eventhi,whatsNew_cat)
	 SELECT m_publish_id, language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,image_file,docs_file,ext_url,content_type,more_photo,m_keyword,m_description,update_date,eventhi,whatsNew_cat FROM importent_information_publish WHERE m_publish_id=$cid";
  mysqli_query($conn,$sql);
		}
	if($row >0)
	{
 $update="UPDATE importent_information_publish,importent_information SET importent_information_publish.m_publish_id = importent_information.m_id,importent_information_publish.language_id = importent_information.language_id, importent_information_publish.cat_id = importent_information.cat_id,importent_information_publish.m_name = importent_information.m_name,importent_information_publish.m_short = importent_information.m_short,importent_information_publish.m_title = importent_information.m_title,importent_information_publish.m_content = importent_information.m_content,importent_information_publish.page_url = importent_information.page_url,importent_information_publish.start_date = importent_information.start_date,importent_information_publish.end_date = importent_information.end_date,importent_information_publish.approve_status = importent_information.approve_status,importent_information_publish.admin_id = importent_information.admin_id,importent_information_publish.c_new_status = importent_information.c_new_status,importent_information_publish.create_date = importent_information.create_date,importent_information_publish.docs_file = importent_information.docs_file,importent_information_publish.ext_url = importent_information.ext_url,importent_information_publish.content_type = importent_information.content_type,importent_information_publish.m_keyword = importent_information.m_keyword,importent_information_publish.m_description = importent_information.m_description,importent_information_publish.update_date = importent_information.update_date WHERE importent_information_publish.m_publish_id =importent_information.m_id and importent_information.m_id=$cid";
  mysqli_query($conn,$update);
	}
	else {
$sql="INSERT INTO importent_information_publish (m_publish_id,language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,more_photo,m_keyword,m_description,update_date)
	 SELECT m_id, language_id,cat_id,m_name,m_short,m_title,m_content,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description,update_date
   FROM importent_information WHERE m_id=$cid";
  mysqli_query($conn,$sql);
  }
}
		$user_id=$_SESSION['admin_auto_id_sess'];
...
<title>Manage Importent Information : <?php echo $sitename;?></title>
<!-- admin css  -->
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_importent_information.php" title="Manage Circular / Events">Manage Importent Information</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Add/Update Importent Information</span>	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Add/Update Important Information </h3>
...
<form action="" method="post" name="form1"  autocomplete="off" enctype="multipart/form-data"  onsubmit="return add_cp('form1')">
	...
	if($_GET['editid']!='')
	{
		$rq = mysqli_query($conn,"select * from importent_information where 	m_id='".$_GET['editid']."'");
...
<?php 
foreach($info_category as $key=>$value)
{
	?>
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_importent_information.php';" />
         </span>
              <div class="clear"></div>
            </div>
