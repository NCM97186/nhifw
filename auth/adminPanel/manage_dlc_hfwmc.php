...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					...
					$sql="Delete From dlc_hfwmc where id='$deleteid'";
					$res=mysqli_query($conn,$sql);
					$page_id=mysqli_insert_id($conn);
					$sql1="Delete From dlc_hfwmc_publish where publish_id='$deleteid'";
...
if(($inactiveid !=''))
{
   ...
   $sql="Update dlc_hfwmc set approve_status='1' where id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update dlc_hfwmc_publish set approve_status='1' where publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
	header("location:manage_dlc_hfwmc.php?status=employee_inactiveid");
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
  $query ="select * from dlc_hfwmc where 1 $querywhere ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
//  $query ="select * from dlc_hfwmc where $wherecluse ORDER BY page_postion ASC ";
  $query ="select * from dlc_hfwmc where 1 ORDER BY page_postion ASC ";
...
<a href="action_dlc_hfwmc.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_dlc_hfwmc.php?inactiveid=<?php echo $id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('dlc_hfwmc_view.php?page_id=<?php echo base64_encode($id);?>','window','width=1000,height=600,scrollbars=yes')"> View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="action_dlc_hfwmc.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_dlc_hfwmc.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
