...
if($deleteid !='')
{
        ...
        if($check_status >0)
        {
         ...
         $sqd = "select image_file FROM whatsnew WHERE m_id=$cid";
        $sql="Delete From whatsnew where m_id='$deleteid'";
        $res=mysqli_query($conn,$sql);
		$sql1="Delete From whatsnew_publish where m_publish_id='$deleteid'";
   ...
   unlink($image_path2);
					...
					if($res)
					{
					header("location:delete.php?status=whatsnew_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update whatsnew set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update whatsnew_publish set approve_status='1' where m_publish_id='$inactiveid'";

 $res=mysqli_query($conn,$sql);
	if($res)
	{
	header("location:delete.php?status=whatsnew_inactiveid");
	}
}
...
<?php
	if($btnsubmit=="Search")
	{
		...
		elseif(!is_numeric($txtstatus)) {
		$errmsg .="Please select correct status.<br>";
		}
		else {
  ...
  $querywhere .=" and approve_status=$txtstatus";
	...
	if($btneng!='')
	  ...
	  {
	  $date=date('Y-m-d');
 // $query ="select * from whatsnew where 1  $querywhere && date(end_date ) <= '$date' ORDER BY page_postion ASC ";
  $query ="select * from whatsnew where 1  $querywhere ORDER BY page_postion ASC ";
...
if(isset($_SESSION['department']) && $_SESSION['department'] > 0){
        $wherecluse .=" and department_id=$_SESSION[department]";
}
$date=date('Y-m-d');
// $query ="select * from whatsnew where 1 $wherecluse && date(end_date ) <= '$date' ORDER BY page_postion ASC ";
 $query ="select * from whatsnew where 1 ORDER BY page_postion ASC ";
}
?>
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">What's New</h3>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>
    ...
    <div id="response"> </div>
							...
							<a href="manage_whatsnew.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('whatsnewview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=1000,height=600,scrollbars=yes')">View </a>
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="whatsnew.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_whatsnew.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
