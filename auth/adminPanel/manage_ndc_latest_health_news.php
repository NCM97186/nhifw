...
$cat_id='1';
$tableName_send="ndc_latest_health_news";
$tableFieldsName_old=array("language_id","m_description","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date");
$tableFieldsValues_send=array("$txtlanguage","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$txtuplodepdf","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date");
$page_id=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);
// $page_id=mysqli_insert_id($conn);
if($txtstatus=='3')
{
$tableName_send="ndc_latest_health_news_publish";
...
header("location:manage_ndc_lhn.php");	
exit;	
}
}
if(isset($_POST['cmdsubmit']) && $_GET['editid']!='')
...
if($errmsg == '')
{
  if($_SESSION['logtoken']!=$_POST['random'])
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
		{
			$sql = "select docs_file FROM ndc_latest_health_news WHERE id=$cid";
...
$old = array("language_id","m_description","m_title","m_content","page_url","ext_url","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date");

$new = array("$txtlanguage","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date");
...
$sql=mysqli_query($conn,"Select * from ndc_latest_health_news_publish $whereclause");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from ndc_latest_health_news where id='".$_GET['editid']."'");
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_ndc_lhn.php';" />
         </span>
              <div class="clear"></div>
            </div>
