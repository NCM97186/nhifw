...
if($txtstatus=='3')
{
$tableName_send="dlc_hmc_publish";
...
header("location:manage_dlc_hmc.php");
exit;	
}
}
if(isset($_POST['cmdsubmit']) && $_GET['editid']!='')
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
		...
		if ($_FILES["txtuplodepdf"]["name"]!="")
			{
			$sql = "select docs_file FROM dlc_hmc WHERE id=$cid";
...
$sql=mysqli_query($conn,"Select * from dlc_hmc_publish $whereclause");
$row=mysqli_num_rows($sql);
	...
	$whereclause="publish_id='$cid'";
                $old = array("language_id","publish_id","m_description","category","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date","category");
                $new = array("$txtlanguage","$cid","$sortcontentdesc","$category","$title","$txtcontentdesc","$url","$exturl","$txtuplodepdf","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date","0");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn, "select * from dlc_hmc where id='".$_GET['editid']."'");
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_dlc_hmc.php" title="Manage What's New">Manage Employee Corner</a></span>
			 <span class="submenuclass">>> </span> 
   ...
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<option value="<?php echo content_desc($row['state_id']);?>" <?php if ($rr['approve_status']==$row['state_id']) echo 'selected="selected"';?>><?php echo $row['state_name']; ?></option>
			...
			{
			$sql=mysqli_query("select * from content_state");
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_dlc_hmc.php';" />
         </span>
              <div class="clear"></div>
            </div>
