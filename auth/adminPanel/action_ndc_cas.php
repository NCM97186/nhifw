...
$cat_id='1';
$tableName_send="ndc_current_awareness_service";
$tableFieldsName_old=array("language_id","m_description","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date","category");
$tableFieldsValues_send=array("$txtlanguage","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$txtuplodepdf","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","0");
$page_id=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);
// $page_id=mysqli_insert_id($conn);
if($txtstatus=='3')
{
$tableName_send="ndc_current_awareness_service_publish";
$tableFieldsName_old=array("publish_id","language_id","m_description","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date","category");
$tableFieldsValues_send=array("$page_id","$txtlanguage","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$txtuplodepdf","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","0");
...
header("location:manage_ndc_current_awareness_services.php");	
exit;	
}
}
if(isset($_POST['cmdsubmit']) && $_GET['editid']!='')
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
		{
			...
			$image_path = "../../upload/ndc/".$row['docs_file'];
			unlink($image_path);
...
$old = array("language_id","m_description","m_title","m_content","page_url","ext_url","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date","category");
$new = array("$txtlanguage","$sortcontentdesc","$title","$txtcontentdesc","$url","$exturl","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date","0");
...
$sql=mysqli_query($conn,"Select * from ndc_current_awareness_service_publish $whereclause");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from ndc_current_awareness_service where id='".$_GET['editid']."'");
...
<title><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?> Latest Health News Publish: <?php echo $sitename; ?></title>
<!-- admin css  -->
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_ndc_lhn.php" title="Manage What's New">Manage NDC  </a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass"><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?>  Latest Health News</span>
	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser"><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?> Latest Health News</h3>
