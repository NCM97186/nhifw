...
$projectcategory= content_desc(check_input($_POST['projectcategory']));
...
if($txtlanguage=='2')
		{
			...
			if(trim($texttype)=="")
			{
			$errmsg.="Please Select project Content Type."."<br>";
		...
		if(trim($txtename)=="")
		{
		$errmsg = "project Title must be Alphanumeric and Special ."."<br>";
		}
		if(trim($txtename)!="")
		{		
		/* if (preg_match('/[^a-zA-Z0-9\s\w-&]/i',$txtename))
			{
				$errmsg .= "project Title must be Alphanumeric and Special Characters( _.,:()&amp ) that should be minimum 2 and maximum 100."."<br>";
			}
			*/
		}
...
if ($_FILES["txtuplode"]["name"]!="")
			...
			$sql = "select doc_uplode FROM project WHERE m_id=$cid";
			...
			$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
				...
				{
				$whereclause="m_id='$cid'";
				$sql=mysqli_query($conn,"Select * from project where $whereclause");
	...
	$old =array("m_flag_id","m_type","language_id","m_name","m_url","m_title","m_keyword","m_description","content","doc_uplode","linkstatus","approve_status","admin_id");
	$new =array("$projectcategory","$texttype","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$txtstatus","$user_id");
	$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
	$page_id=$cid;
...
if($txtstatus=='3')
{
	...
	$tableName_send="project_publish";
	$whereclause="where m_publish_id='$cid'";
    $sql=mysqli_query($conn,"Select * from project_publish $whereclause");
    $row=mysqli_num_rows($sql); 
	if($row >0)
	{
		
	$whereclause="m_publish_id='$cid'";
	$old =array("m_type","m_flag_id","language_id","m_name","m_url","m_title","m_keyword","m_description","content","doc_uplode","linkstatus","approve_status","admin_id");
	$new =array("$texttype","$projectcategory","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$txtstatus","$user_id");
$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
...
$value=$useAVclass->insertQuery($tableName,$tableFieldsName_send,$tableFieldsValues_send);
	...
	$tableFieldsName_send=array("m_type","m_publish_id","m_flag_id","language_id","m_name","m_url","m_title","m_keyword","m_description","content","doc_uplode","linkstatus","approve_status","admin_id","create_date");
	$tableFieldsValues_send=array("$texttype","$page_id","$projectcategory","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$txtstatus","$user_id","$create_date");
...
$_SESSION['token'] = "";
$_SESSION['uniq'] = "";
header("location:manage_project.php?txtstatus=" .$txtstatus);
}

}
$editid = $_GET['editid'];
 $edit_contrator ="select * from project where m_id='$editid'";
...
$fetch_result= @extract($fetch_result);
...
<title> Edit Project Page : <?php echo $sitename;?></title>
...
<div class="breadcrum">
  <span class="subprojectclass"><a href="welcome.php">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_project.php?module_id=1">Manage project</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Edit project</span>
			</span>
</div>
<div class="clear"> </div>
</div>       
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Edit project</h3>
<!-- <div class="right-section">
  <ul>
<?php if($role_map['draft']=='DR' || $user_id=='101'){?>
 <li  class="new-icon"><a href="add_project.php?module_id=<?php echo $model_id;?>" title="New"><span class="icon-28-new"></span>New</a>            </li><?php }?>
...
<div class="frm_row"> <span class="label1">
              <label for="txtename">Project Title:</label>
...
<div id="content1">
<div class="frm_row"> <span class="label1">
                    <label for="projectcategory">Primary Link:</label>
                    <span class="star">*</span></span> <span class="input1">
                    
            <?php
			
$nav_query = mysqli_query($conn,"select * from project where approve_status='3' and m_flag_id='0' and language_id='$language_id'");
...
{ 
	...
	include("../../includes/connection.php");
     ...
     $child_query = mysqli_query($conn,"select * from project where approve_status='3' and m_flag_id='$oldID'");
	 ...
	 while ( $child = mysqli_fetch_array($child_query) )
     {
          if ( $child['m_id'] != $child['m_flag_id'] )
          {
			  ...
			  // <option value="'.$nav_row['division_id'].'">'.$nav_row['project_name'].'</option>
...
echo '<select id="projectcategory" name="projectcategory">'.$tree.'</select>';
...
<div class="frm_row"> <span class="label1">
<label for="texttype">Project Type :</label>
<span class="star">*</span></span> <span class="input1">
...
<?php $con="select * from project_publish where m_flag_id ='0'  and menu_positions	='1' and language_id='$lan' and approve_status='3' ORDER BY m_publish_id";
$sql=mysqli_query($conn,$con);
$counter=mysqli_num_rows($sql);
 $footercon="select * from project_publish where m_flag_id ='0'  and menu_positions	='3' and language_id='$lan' and approve_status='3' ORDER BY m_publish_id";
...
<input type="button" class="button" value="Back" onClick="javascript:location.href = 'manage_project.php'" />
</span>
<div class="clear"></div>
</div> 
</form>
