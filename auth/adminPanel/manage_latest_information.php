...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
 			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM latest_information WHERE m_id=$deleteid";
			 $page_id=mysqli_insert_id($conn);
					$sql="Delete From latest_information where m_id='$deleteid'";
					$res=mysqli_query($conn,$sql);
					$sql1="Delete From latest_information_publish where m_publish_id='$deleteid'";
...
<?php 
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  {
	 $date=date('Y-m-d');
   //$query ="select * from latest_information where 1  $querywhere && date(end_date ) >= '$date'  ORDER BY page_postion ASC "; 

    $query ="select * from latest_information where 1  $querywhere    ORDER BY page_postion ASC "; 
	  }
	 else
	{
...
$date=date('Y-m-d');
// $query ="select * from latest_information where 1 $wherecluse && date(end_date ) >= '$date'  ORDER BY page_postion ASC ";
$query ="select * from latest_information where 1  ORDER BY page_postion ASC ";
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Meeting / Workshop</h3>
 <div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="latest_information.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<a href="manage_latest_information.php?inactiveid=<?php echo $m_id; ?>&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')"title="Inactive">Inactive</a>&nbsp;/&nbsp;<?php }?>
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('latest_informationview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
jQuery(document).ready(function(){ 	
  ...
  jQuery("#response").slideUp("slow", function () {
	...
	jQuery("#list ul").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			var order = jQuery(this).sortable("serialize") + '&update=update' + '&tab=latest_information'; 
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="latest_information.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_latest_information.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
