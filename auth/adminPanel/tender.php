...
if(isset($cmdsubmit) && $_GET['editid']=='')
{
       ...
       // $txtename = check_input($_POST['text_title']);
        ...
        $sub_category = content_desc(check_input($_POST['sub_category']));      
        $content_type = content_desc(check_input($_POST['content_type']));      
        ...
        $supp_doc= content_desc(check_input($_POST['supp_doc']));
	...
	$errmsg .="Please Select Language."."<br>";
		...
		$errmsg .="Please enter Short Description."."<br>";
                                ...
                                if ($_FILES["supp_doc"]["tmp_name"]!="")
		{
                                        $tempfile=($_FILES["supp_doc"]["tmp_name"]);
                                        $imageinfo = ($_FILES["supp_doc"]["type"]);
...
if($errmsg == '')
	{
		...
		exit();
                                ...
                                if ($_FILES["supp_doc"]["name"]!="")
		{
                                        $txtuplodepdf=$_FILES['supp_doc']['name'];
                                        $txtuplode = preg_replace("/[^a-zA-Z0-9.]/", "", $txtuplodepdf);
                                        $uniq = uniqid("");
                                        $txtuplodepdf=$uniq.$txtuplodepdf;		
                                        $PATH="../../upload/publication/";					
                                        $PATH=$PATH."/"; 
                                        $val=move_uploaded_file($_FILES["supp_doc"]["tmp_name"],$PATH.$txtuplodepdf);
...
$tableFieldsName_old=array("language_id","m_description","category","sub_category","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date");
$tableFieldsValues_send=array("$txtlanguage","$sortcontentdesc","$category","$sub_category","$title","$txtcontentdesc","$url","$exturl","$doc_file","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date");
...
if($txtstatus=='3')
{
	...
	$tableFieldsName_old=array("publish_id","language_id","m_description","category","sub_category","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date");
               $tableFieldsValues_send=array("$page_id","$txtlanguage","$sortcontentdesc","$category","$sub_category","$title","$txtcontentdesc","$url","$exturl","$doc_file","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date");
...
$title = content_desc(check_input($_POST['text_title']));
        ...
        $sub_category = check_input($_POST['sub_category']);      
        $content_type = check_input($_POST['content_type']);      
        $sortcontentdesc= content_desc(check_input($_POST['sortcontentdesc']));
        $txtcontentdesc= check_input($_POST['txtcontentdesc']);
        $supp_doc= check_input($_POST['supp_doc']);
        $txtstatus=content_desc(check_input($_POST['txtstatus']));
...
if($txtlanguage=='2')
{
		...
		if ($_FILES["supp_doc"]["name"] != "")
		{
		if ($_FILES["supp_doc"]["size"] >=1048576)
		{
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
		...
		if ($_FILES["supp_doc"]["name"]!="")
			{
			$sql = "select docs_file FROM publication WHERE id=$cid";
			$rs = mysqli_query($conn,$sql);
			$row=mysqli_fetch_array($rs);
			$image_path = "../../upload/publication/".$row['docs_file'];
			unlink($image_path);
...
$old = array("language_id","m_description","category","sub_category","m_title","m_content","page_url","ext_url","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date");
$new = array("$txtlanguage","$sortcontentdesc","$category","$sub_category","$title","$txtcontentdesc","$url","$exturl","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date");

$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
//if($txtstatus=='3'){
	
                $tableName_send="publication_publish";
	$whereclause="where publish_id='$cid'";
$sql=mysqli_query($conn,"Select * from publication_publish $whereclause");
$row=mysqli_num_rows($sql);
	...
	$whereclause="publish_id='$cid'";
                $old = array("language_id","publish_id","m_description","category","sub_category","m_title","m_content","page_url","ext_url","docs_file","start_date","end_date","approve_status","content_type","admin_id","create_date","update_date");
                $new = array("$txtlanguage","$cid","$sortcontentdesc","$category","$sub_category","$title","$txtcontentdesc","$url","$exturl","$txtuplodepdf","$startdate","$expairydate","$txtstatus","$content_type","$user_id","$create_date","$update_date");
...
if($_GET['editid']!='')
{
        $rq = mysqli_query($conn,"select * from publication where id='".$_GET['editid']."'");
...
//echo 'select m_id from menu_publish where approve_status=3 and language_id='.$language_id.' and m_name="Publications"';
$sql= mysqli_query($conn,'select m_id from menu_publish where approve_status=3 and language_id='.$language_id.' and m_id='.$pub_id);
while($row = mysqli_fetch_assoc($sql))
{
        $menu_id = $row['m_id'];
}
//echo "select * from menu_publish where approve_status=3 and language_id='$language_id' and m_flag_id=$menu_id";
$sql1= mysqli_query($conn,"select * from menu_publish where approve_status=3 and language_id='$language_id' and m_flag_id=$menu_id");
while($row1 = mysqli_fetch_assoc($sql1))
{
        $categoryList[] = $row1;
       //echo   "select * from menu_publish where approve_status=3 and m_flag_id=$row1[m_id]";
        $sql2= mysqli_query($conn,"select * from menu_publish where approve_status=3 and language_id='$language_id' and m_flag_id=$row1[m_id]");
...
<title><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?> Publication: <?php echo $sitename; ?></title>
<!-- admin css  -->
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_publication.php" title="Manage What's New">Manage Publication</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass"><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?> Publication</span>
	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser"><?php if($_GET['editid'] !=''){ echo "Update"; } else { echo "Add"; } ?> Publication</h3>
...
<div class="frm_row"> <span class="label1">
              ...
              <span class="star">*</span></span> <span class="input1">
                  <input type="radio" name="txtlanguage" autocomplete="off" value="1" <?php if($rr['language_id']=='1'){ echo 'checked'; } else echo 'checked'; ?> id="txtlanguage" onclick="getPublicationCategory(this.value);" />English &nbsp;<input id="txtlanguage"  type="radio" name="txtlanguage" autocomplete="off" value="2" <?php if($rr['language_id']=='2'){ echo 'checked'; } ?> onclick="getPublicationCategory(this.value);" />Hindi 
               </span>
              <div class="clear"></div>
     ...
     <div class="frm_row"> <span class="label1">
                ...
                <span class="star">*</span></span> <span class="input1">
                    <select name="category" id="category">
                            ...
                            <option value="<?php echo content_desc($cat['m_id']); ?>" <?php if($rr['category']==$cat['m_id']) { ?> selected <?php } ?>><?php echo $cat['m_name']; ?></option>
                            <?php } ?>
                    </select>
...
<!-- <div class="frm_row"> <span class="label1">
                ...
                <label for="content_type">Content Type:</label>
                 <span class="star">*</span>
                    ...
                    <select name="content_type" id="content_type">
                            <option value="">Select</option>
                            <option value="3" <?php  if($rr['content_type']=='3'){ echo 'selected';}  ?>>URL</option>
                            <option value="2" <?php  if($rr['content_type']=='2'){ echo 'selected';}  ?>>PDF</option>
                            <option value="1" <?php  if($rr['content_type']=='1'){ echo 'selected';}  ?>>HTML</option>
                    </select>
    ...
    <div class="frm_row" id="url_div" style="<?php if($rr['content_type'] == '3'){ echo 'display:block;'; } else { echo 'display:none;'; } ?>"> <span class="label1">
              <label for="page_url">External Link:</label>
              </span> <span class="input1">
              <input type="text" name="page_url" id="page_url" autocomplete="off" value="<?php echo $rr['ext_url']; ?>"/>
               </span>
              <div class="clear"></div>
			  <div class="loading"></div>
            </div> 
    
    <div class="frm_row" id="pdf_div" style="<?php if($rr['content_type'] == '2'){ echo 'display:block;'; } else { echo 'display:none;'; } ?>"> <span class="label1">
                                                        <label for="supp_doc">Supporting Document</label><span class="star">*</span>
                                                    </span> <span class="input1">
                                          <input type="file" name="supp_doc" id="supp_doc" autocomplete="off" value=""/><span><?php echo $rr['docs_file']; ?></span>

                                                    </span>
                                                    <div class="clear"></div>
            </div> 
    
    <!--<div class="frm_row" id="pdf_div" style="<?php if($rr['content_type'] == 'pdf'){ echo 'display:block;'; } else { echo 'display:none;'; } ?>"> <span class="label1">
              <label for="newicons">Upload Document:</label>
              </span> <span class="input1">
                  <input type="file" name="txt_doc" id="txt_doc" autocomplete="off" value=""/> <span><?php echo $rr['upload_doc']; ?></span>
               </span>
              <div class="clear"></div>
			  <div class="loading"></div>
            </div>  -->
    ...
    <div class="frm_row" style="<?php if($rr['content_type'] == '1'){ echo 'display:block;'; } else { echo 'display:none;'; } ?>" id="html_div"> <span class="label1">
...
<?php
		...
		$ckeditor->config['filebrowserBrowseUrl'] = ''.$HomeURL.'/auth/adminPanel/ckfinder/ckfinder.html';
		$ckeditor->config['filebrowserImageBrowseUrl'] = ''.$HomeURL.'/auth/adminPanel/ckfinder/ckfinder.html?type=Images';
		$ckeditor->config['filebrowserUploadUrl'] = ''.$HomeURL.'/auth/adminPanel/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Files';
		$ckeditor->config['filebrowserImageUploadUrl'] = ''.$HomeURL.'/auth/adminPanel/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Images';
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_publication.php';" />
         </span>
              <div class="clear"></div>
            </div>
