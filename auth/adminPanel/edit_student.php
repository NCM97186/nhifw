...
if($txtlanguage=='2')
		{
			...
			if(trim($texttype)=="")
			{
			$errmsg.="Please Select student Content Type."."<br>";
		...
		if(trim($txtename)=="")
		{
		$errmsg = "student Title must be Alphanumeric and Special ."."<br>";
		}
		if(trim($txtename)!="")
		{		
...
if ($_FILES["txtuplode"]["name"]!="")
			...
			$sql = "select doc_uplode FROM student WHERE m_id=$cid";
			...
			$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
				...
				{
				$whereclause="m_id='$cid'";
				$sql=mysqli_query($conn,"Select * from student where $whereclause");
	...
	$new =array("$studentcategory","$texttype","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$update_date","$txtstatus","$user_id");
...
$sql=mysqli_query($conn,"Select * from student_publish $whereclause");
$row=mysqli_num_rows($sql); 
	...
	$new =array("$texttype","$studentcategory","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$update_date","$txtstatus","$user_id");
$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);
...
$value=$useAVclass->insertQuery($tableName,$tableFieldsName_send,$tableFieldsValues_send);
	...
	$tableFieldsValues_send=array("$texttype","$page_id","$studentcategory","$txtlanguage","$txtename","$url","$page_url","$txtekeyword","$txtmeta_description","$txtcontentdesc","$txtuplode","$link","$update_date","$txtstatus","$user_id");
...
$_SESSION['token'] = "";
$_SESSION['uniq'] = "";
header("location:manage_student.php?txtstatus=" .$txtstatus);
}

}
$editid = base64_decode($_GET['editid']);
 $edit_student ="select * from student where m_id='$editid'";
$student_result = mysqli_query($conn,$edit_student);
$res_rows=mysqli_num_rows($student_result);
$fetch_result=mysqli_fetch_array($student_result);
...
<title> Edit Student Corner: <?php echo $sitename;?></title>
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_student.php?module_id=1">Manage Student Corner</a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Edit Student Corner</span>
			</span>
</div>
<div class="clear"> </div>
</div>       
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Edit Student Corner</h3>
<!-- <div class="right-section">
  <ul>
<?php if($role_map['draft']=='DR' || $user_id=='101'){?>
 <li  class="new-icon"><a href="add_student.php?module_id=<?php echo $model_id;?>" title="New"><span class="icon-28-new"></span>New</a>            </li><?php }?>
