...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
 			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM latest_information WHERE m_id=$cid";
...
<?php 
	...
	$texcat1 = content_desc(check_input($texcat));
	$txtstatus1 = content_desc(check_input($txtstatus));

	if($btnsubmit=="Search")
	{
		if($filter_search!='')
		{
				if(preg_match("/^[aA-zZ][a-zA-Z -]{2,20}+$/", content_desc(check_input($filter_search))) == 0)
				{
  ...
  $querywhere .=" and approve_status=$txtstatus1"; 
