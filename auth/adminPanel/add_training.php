...
$texttype= check_input($_POST['texttype']);
  ...
  $texttype_training= check_input($_POST['texttype_training']);

 
 
$txtcoordinator =  check_input($_POST['txtcoordinator']);
...
if($txtlanguage=='2')
{
		...
		if(trim($texttype_training)=='0') {
		

		$errmsg .="Please select Training option."."<br>";

		}

		if(trim($texttype_training) !== "" ) 
		{
					
					if(trim($txtcoordinator) == "" )
					{
						$errmsg .="Please enter coordinator name."."<br>";
					}
		}		
...
$tableName_send="tbl_training";

$tableFieldsName_old=array("language_id","cat_id","m_name","m_short","m_title","m_content","m_type","coordinator","page_url","start_date","end_date","approve_status","admin_id","c_new_status","create_date","docs_file","ext_url","content_type","m_keyword","m_description");
$tableFieldsValues_send=array("$txtlanguage","$cat_id","$txtename","$sortcontentdesc","$m_title","$txtcontentdesc","$texttype_training","$txtcoordinator","$url","$startdate","$expairydate","$txtstatus","$user_id","$newicons","$createdate","$txtuplodepdf","$txtweblink","$texttype","$txtekeyword","$txtmeta_description");
$page_id=$useAVclass->insertQuery($tableName_send,$tableFieldsName_old,$tableFieldsValues_send);


if($txtstatus=='3')
{
	$sql="INSERT INTO tbl_training_publish (m_publish_id,language_id,cat_id,m_name,m_short,m_title,m_content,m_type,coordinator,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description)
	 SELECT m_id, language_id,cat_id,m_name,m_short,m_title,m_content,m_type,coordinator,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description
   FROM tbl_training WHERE m_id=$page_id";
...
if($txtlanguage=='2')
{
		...
		if($texttype_training == "0")
		{
			$errmsg .="Please Select training Type."."<br>";
		}

		if(trim($texttype_training)=="1"  && trim($txtcoordinator) == "" )
		{
			 	 $errmsg .="Please enter coordinator name."."<br>";
		}
...
if($errmsg == '')
  ...
  if($_SESSION['logtoken']!=$_POST['random'])
	...
	if ($_FILES["txtuplodepdf"]["name"]!="")
			...
			$sql = "select docs_file FROM tbl_training WHERE m_id=$cid";
...
$old=array("language_id","m_name","cat_id","m_short","m_title","m_content","m_type","coordinator","page_url","start_date","end_date","approve_status","admin_id","c_new_status","create_date","ext_url","content_type","m_keyword","m_description","update_date");

$new=array("$txtlanguage","$txtename","$cat_id","$sortcontentdesc","$m_title1","$txtcontentdesc","$texttype_training","$txtcoordinator","$url","$startdate","$expairydate","$txtstatus","$user_id","$newicons","$create_date","$txtweblink","$texttype","$txtekeyword","$txtmeta_description","$update_date");
	$useAVclass->UpdateQuery($tableName_send,$whereclause,$old,$new);

if($txtstatus=='3')
{
	$tableName_send="tbl_training_publish";
	$whereclause="where m_publish_id='$cid'";
$sql=mysqli_query($conn,"Select * from tbl_training_publish $whereclause");
$row=mysqli_num_rows($sql); 
	if($row >0)
	{
 $update="UPDATE tbl_training_publish,tbl_training SET tbl_training_publish.m_publish_id = tbl_training.m_id,tbl_training_publish.language_id = tbl_training.language_id, tbl_training_publish.cat_id = tbl_training.cat_id,tbl_training_publish.m_name = tbl_training.m_name,tbl_training_publish.m_short = tbl_training.m_short,tbl_training_publish.m_title = tbl_training.m_title,tbl_training_publish.m_content = tbl_training.m_content,tbl_training_publish.m_type = tbl_training.m_type,tbl_training_publish.coordinator  = tbl_training.coordinator,tbl_training_publish.page_url = tbl_training.page_url,tbl_training_publish.start_date = tbl_training.start_date,tbl_training_publish.end_date = tbl_training.end_date,tbl_training_publish.approve_status = tbl_training.approve_status,tbl_training_publish.admin_id = tbl_training.admin_id,tbl_training_publish.c_new_status = tbl_training.c_new_status,tbl_training_publish.create_date = tbl_training.create_date,tbl_training_publish.docs_file = tbl_training.docs_file,tbl_training_publish.ext_url = tbl_training.ext_url,tbl_training_publish.content_type = tbl_training.content_type,tbl_training_publish.m_keyword = tbl_training.m_keyword,tbl_training_publish.m_description = tbl_training.m_description,tbl_training_publish.update_date = tbl_training.update_date WHERE tbl_training_publish.m_publish_id =tbl_training.m_id and tbl_training.m_id=$cid";
  mysqli_query($conn,$update);
	}
	else {
$sql="INSERT INTO tbl_training_publish (m_publish_id,language_id,cat_id,m_name,m_short,m_title,m_content,m_type,coordinator,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,more_photo,m_keyword,m_description,update_date)
	 SELECT m_id, language_id,cat_id,m_name,m_short,m_title,m_content,m_type,coordinator,page_url,start_date,end_date,approve_status,admin_id,c_new_status,create_date,docs_file,ext_url,content_type,m_keyword,m_description,update_date
   FROM tbl_training WHERE m_id=$cid";
  mysqli_query($conn,$sql);
  }
}
		$user_id=$_SESSION['admin_auto_id_sess'];
...
<div class="breadcrum">
  <span class="submenuclass"><a href="welcome.php" title="Dashboard">Dashboard</a></span>
			...
			<span class="submenuclass"><a href="manage_tbl_training.php" title="Manage Circular / Events">Manage Trainings </a></span>
			 <span class="submenuclass">>> </span> 
			<span class="submenuclass">Add/Update Training </span>	</div>
<div class="clear"> </div>
</div>  
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $errmsg; ?></p>
 ...
 <h3 class="manageuser">Add/Update Training </h3>
...
<form action="" method="post" name="form1"  autocomplete="off" enctype="multipart/form-data"  onsubmit="return add_cp('form1')">
	...
	if($_GET['editid']!='')
	{
		$rq = mysqli_query($conn,"select * from tbl_training where 	m_id='".$_GET['editid']."'");
...
<div class="frm_row"> <span class="label1">
		...
		<div class="frm_row" style="display: none;"> <span class="label1">
...
<div class="frm_row"> <span class="label1">
<label for="texttype">Training Type :</label>
<span class="star">*</span></span> <span class="input1">

<select name="texttype_training" id="texttype_training" autocomplete="off" >
<option value="0" <?php if ($rr['m_type'] == '0') {
	echo 'selected';
}?> >Select</option>
<option value="1" <?php if ($rr['m_type'] == '1') {
	echo 'selected';
}?> >Institutional Trainings</option>
<option value="2" <?php if ($rr['m_type'] == '2') {
	echo 'selected';
}?>>Central Training Plan</option> 
</select>
...
<div class="frm_row" id="coodinate_area" <?php if ($rr['m_type'] == '1'){ echo 'style="display: block;"'; }else{ echo 'style="display: none;"';} ?>
	
  > <span class="label1">
<label for="texttype"> Coordinator Name :</label>
<span class="star">*</span></span> <span class="input1">
<input name="txtcoordinator" autocomplete="off" type="text" class="input_class" id="txtcoordinator"   value="<?php if($rr['coordinator'] !=""){ echo $rr['coordinator'];}else{ echo ""; }?>">
...
<div id="txtweb"<?php if($texttype=='3' || $rr['content_type']=='3') {?> style="display:block" <?php } else {?>style="display:none" <?php } ?>>
   <div class="frm_row"> <span class="label1">
    ...
    <div class="frm_row"> <span class="label1">
		...
		<input type="button" class="button" value="Back" onClick="javascript:location.href ='manage_tbl_training.php';" />
         </span>
              <div class="clear"></div>
            </div>
...
$('#texttype_training').change(function(){
