...
<?php 
	if($btnsubmit=="Search" || $_GET['page_query'] =="Search")
	{
	  ...
	  if($errmsg=='')	
	  {
		 ...
		 }else { $query ="select m_id,m_name,m_title,approve_status from menu  where m_flag_id=0 $querywhere ORDER BY page_postion ASC"; }
	  }
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<?php
								// txtstatus=$txtstatus&page_query=Search&&module_id=".$_GET['module_id']."
							...
							<span class="space-pagetitle"><?php if(showroot($m_id) >0) {?><a href="manage_submenu.php?sub=<?php echo $m_id; ?>&module_id=<?php echo $model_id;?>&&txtstatus=<?php echo $txtstatus;?>" title="Sub Menu View"><?php status($approve_status);?></a><br /><span class="clicksubmenu">(Click to View Submenu)</span><?php } else { status($approve_status); }?></span>
                            ...
                            <a href="" class="cat_link" title="View" onclick="MM_openBrWindow('menuview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=700,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php  }?>
