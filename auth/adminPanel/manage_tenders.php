...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
			 ...
			 $sqd = "select docs_file FROM tender WHERE m_id=$cid";
					$sql="Delete From tender where m_id='$deleteid'";
					$res=mysqli_query($conn,$sql);
						$sql="Delete From tender_publish where m_publish_id='$deleteid'";
					$res=mysqli_query($conn,$sql);
...
if(($inactiveid !=''))
{
   ...
   $sql="Update tender set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update tender_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
...
<a href="tenders.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<label for="startdate" >From Date:</label>
					...
					<input type="text" name="expirydate"  autocomplete="off" id="expirydate" value="<?php echo htmlentities($expirydate);?>" size="10"  onkeypress="return isNumberKey(event)"/>
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
                            ...
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('tenderview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="tenders.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
